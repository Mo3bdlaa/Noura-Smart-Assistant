...
export function assembleSystem(input: AssembleInput): string {
  ...
  blocks.push(
    [
      "تعليمات الرد:",
      "- ردّي كإنسانة حقيقية بالمصري، وطابقي طول رسالته: رسالة قصيرة تتقابل برد قصير. الأصل رسالة واحدة.",
      ...
      "- ابعتي فقاعتين (سطر فاضي) نادراً جداً وبس لما يكون فيه فكرتين مختلفتين فعلاً — مش عشان تكتري كلام.",
      "- إيموجي قليل جداً (أغلب الردود من غير أي إيموجي)، ومتعلنيش حبك أو إنك بتفكري فيه من غير مناسبة.",
    ].join("\n"),
  );
